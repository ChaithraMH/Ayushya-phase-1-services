 ...
 * Date : 01-Aug-2019
 * Job Model class
 * Defines the job service data validation methods
 *
 */
@Component
public class JobDataValidation {
	...
	public boolean validate(JobDataModel jobDataModel) throws InvalidServiceRequestException{
		...
		}else if(jobDataModel.getJobId()==null || (jobDataModel.getJobId()!=null && jobDataModel.getJobId().trim().length()<1)) {
			throw new InvalidServiceRequestException("Invalid job notes request data");
		}else if (jobDataModel.getTicketId()==null || (jobDataModel.getTicketId()!=null && jobDataModel.getTicketId().trim().length()<1)) {
			throw new InvalidServiceRequestException("Invalid ticket request data");
		}else if (jobDataModel.getCustomerId()==null || (jobDataModel.getCustomerId()!=null && jobDataModel.getCustomerId().trim().length()<1)) {
			throw new InvalidServiceRequestException("Invalid customer request data");	
		}else if (jobDataModel.getStatusId()==null || (jobDataModel.getStatusId()!=null && jobDataModel.getStatusId().trim().length()<1)) {
