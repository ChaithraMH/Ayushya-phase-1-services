...
public class NotificationService implements INotificationService {
	 ...
	 * "AC776f26d56f8c79d0d6f5b527ee4918f7"; public static final String AUTH_TOKEN =
	 * "f7ecb5d41f099ecb5f59605b12b69940"; public static final String TWILIO_NUMBER
	 * = "+16176741431";
	 */
//	
//	
...
//		response.setMessage("otp validated successfully...");
		/*
		 * if (communicationEntity1 == false) { response.setMessage("otp invalid ");
		 * response.setStatusCode("400"); return response; } else if
		 * (communicationEntity.getOtp().equals(otp)) {
		 * response.setMessage("otp  validation succesfully"); return response; } else
		 * if (communicationEntity.getOtp().equals(otp) &&
		 * communicationEntity.getOtpExper().equals(afterAddingTenMins)) {
		 * response.setMessage("otp time exper"); response.setErrors("401"); return
		 * response;
		 * 
		 * } else { response.setMessage("otp in valid "); response.getStatus(); return
		 * response;
		 */
		// }
		return response;
...
//			response.setStatus("Invoce generated");
		...
		case "Assign customer":
			sendSMS(number, "A service request has been created and customer service agent "
					...
					+ " at your location on " + communicationModel.getOtpExper() + communicationModel.getLastUpdate());
//			response.setId(0);
